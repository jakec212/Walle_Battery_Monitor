...
const int PWM_PIN = 9;  // Hardware PWM pin
...
void loop() {
  ...
  if (detectedCells < 1) {
    ...
    analogWrite(PWM_PIN, 0);  // 0% duty cycle = no battery
    delay(1000);
    return;
  }
  ...
  //----------------------------------
  // PWM output (0–100% maps to 0–255)
  //----------------------------------

  int pwmValue = map(batteryPercent, 0, 100, 0, 255);
